var VR_Share_Model = (function () {
  ...
  return {
    ...
    isMobile: isMobile,
    shareURI: {
      WAShare: "https://wa.me/?text={text}" /*isMobile
...
var VR_Share_Service = (function () {
  const DB_TOKEN = document.location.href;
  const DB_Schema = { count: 1 };
  function updateShareCount() {
    let DB = JSON.parse(localStorage.getItem(DB_TOKEN));
    if (DB) {
      ++DB.count;
    } else {
      DB = DB_Schema;
  ...
  function getShareCount() {
    const DB = JSON.parse(localStorage.getItem(DB_TOKEN));
...
var VR_Share = (function (model, service) {
  let appID;
  let appToken;
  let shareUrl;
  let maxShare = 0;
  ...
  function init(config) {
    ...
    if (!config.hasOwnProperty("appID"))
      throw new Error("appID is missing in the config object");
    if (!config.hasOwnProperty("appToken"))
      throw new Error("appToken is missing in the config object");
    if (!config.hasOwnProperty("postUrl"))
      throw new Error("postUrl is missing in the config object");
    // Setup configuration data
    appID = config.appID;
    appToken = config.appToken;
    postUrl = config.postUrl;
    shareUrl = encodeURIComponent(config.shareUrl || location.href);
    maxShare = config.maxShare || 5;
    btnOptions.share = config.shareButtonText || btnOptions.share;
    btnOptions.post = config.postButtonText || btnOptions.post;
    // validate the app
    validateApp();
    if (isValid) runApp();
  }
  ...
  function createAppElements() {
    const appDiv = `
      <div class="vr-d-flex vr-flex-wrap vr-align-items-center vr-justify-content-center">

      </div>
    `;
    const waShareBtn = document.createElement("button");
    const msgShareBtn = document.createElement("button");
    const postBtn = document.createElement("button");
    const progressBar = document.createElement("div");
    const progress = document.createElement("div");
    const progressText = document.createElement("span");
    const shareBtnPanel = document.createElement("div");
  ...
  function createToaster() {
    ...
    const toasterPanel = document.createElement("div");
    const toasterBody = document.createElement("div");
  ...
  function updateProgress() {
    ...
    const shareCount = service.getShareCount();
    const percent = (shareCount / maxShare) * 100;
  ...
  function shareOnWhatsApp() {
    if (service.getShareCount() >= maxShare)
      return showToaster("Share completed!!!", 2000);
    let url = model.shareURI.WAShare.replace("{text}", shareUrl);
    const tab = openWindow(url, "Share on Whatsapp", 400, 300);
    handleClose(tab);
  }
  ...
  function shareOnFBMessenger() {
    if (service.getShareCount() >= maxShare)
      return showToaster("Share completed!!!", 2000);
    let url = model.shareURI.FBMSGShare.replace("{link}", shareUrl);
    const tab = openWindow(url, "Share on Facebook Messenger", 400, 300);
    handleClose(tab);
  }

  function trackShare() {
    if (service.getShareCount() >= maxShare) return;
    service.updateShareCount();
    updateProgress();
    service.getShareCount() >= maxShare ? enablePostBtn() : null;
  }

  function canViewPost() {
    return service.getShareCount() === maxShare;
  }

  function enablePostBtn() {
    if (canViewPost()) {
      const postBtn = document.querySelector("#vr-post-btn");
      if (postBtn) {
...
})(VR_Share_Model, VR_Share_Service);
